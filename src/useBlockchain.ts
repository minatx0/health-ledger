...
const useWeb3 = () => {
    ...
    const loadBlockchainData = async () => {
      try {
        if (window.ethereum) {
          ...
          if (accounts.length === 0) throw new Error('No accounts. Please connect to MetaMask.');
  ...
  const sendTransaction = async (methodName: string, params: any[], value = '0') => {
    if (!state.web3 || !state.accounts) throw new Error('Web3 or accounts not initialized');
    let result = null;

    try {
      const gas = await state.contract.methods[methodName](...params).estimateGas({ from: state.accounts[0] });
      result = await state.contract.methods[methodName](...params).send({ from: state.accounts[0], gas, value });
    } catch (error) {
